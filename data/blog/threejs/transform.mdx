...
![](/static/images/threejs/transform.png)
...
rotation 屬性是 Euler 類型，控制物件的旋轉，對應 x、y 和 z 三個軸的旋轉角度（以弧度表示）：
...
當場景包含多個物件時，可以將它們分組，方便進行整體操作。群組是 Group 類型，繼承自 Object3D。
...
總結
這些程式碼示範了如何使用 Three.js 的基本變形功能來操控 3D 場景中的物件，從簡單的移動到群組操作，為後續的動畫和更複雜的應用打下基礎。
